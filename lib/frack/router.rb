...
def call(env)
    ...
    def delete(route)
      self.routes.merge!(route.keys.first + 'DELETE' => route.values.first  )
    end
    
    def patch(route)
      self.routes.merge!(route.keys.first + 'PATCH' => route.values.first  )
    end

    def post(route)
      self.routes.merge!(route.keys.first + 'POST' => route.values.first  )
    end
